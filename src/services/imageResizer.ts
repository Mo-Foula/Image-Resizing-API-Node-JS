...
export const imageResizer = async function (
  filename: string,
  height: number,
  width: number
): Promise<string> {
  try {
    const path = pathP.resolve(
      `images/thumb/${filename}_${height}_${width}.jpg`
    )
    ...
    if (!existingPathChecker(path)) {
      const buffer: any = await resizeImage(filename, height, width)
      if (!buffer) {
